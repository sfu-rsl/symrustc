...
COPY --chown=ubuntu:ubuntu . $HOME/belcarra_source0
...
#
RUN cd ~/belcarra_source0/src/rs \
    && rm -rf rust_source \
    && tar xf ~/belcarra_source0/rust_source.tar \
    && rm ~/belcarra_source0/rust_source.tar

RUN tar xf ~/belcarra_source0/symcc_source_main.tar \
    && rm ~/belcarra_source0/symcc_source_main.tar
